...
void MODEL(const Instance& inst, Solution& sol, const int& level) {
	...
	model.getEnv().set(GRB_IntParam_PoolSearchMode, 2);
	model.getEnv().set(GRB_IntParam_PoolSolutions, 10000);
	model.getEnv().set(GRB_DoubleParam_PoolGap, 0);	
	
    // find the optimal solution	
    model.optimize();

    // store the results in a Solution object
	if(level == 1) sol.C1 = model.get(GRB_IntAttr_SolCount);
	if(level == 2) sol.C2 = model.get(GRB_IntAttr_SolCount); 
