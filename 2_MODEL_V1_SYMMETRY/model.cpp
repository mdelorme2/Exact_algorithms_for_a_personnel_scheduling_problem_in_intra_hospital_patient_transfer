...
void MODEL(const Instance& inst, Solution& sol, const int& level, const int& rule) {
	...
	vector<GRBLinExpr> totI(inst.nbE, 0);    	
	vector<GRBLinExpr> sA(inst.nbHours * (1+inst.sdmx-inst.sdmn),0);
    
	for (int i = 0; i < inst.nbE; i++) { 
		for (int j = 0; j < inst.shifts.size(); j++) {
			totI[i] += x[i][j] *inst.shifts[j][inst.nbTS];
			totS[i] += x[i][j];
	...
	if (level == 2){
		...
		for (int i = 0; i < inst.nbHours * (1+inst.sdmx-inst.sdmn); i++) {
			model.addConstr(sA[i] <= inst.nbE * inst.nbDays * y[i]);
			obj2+= y[i];
		}
		for (int i = 0; i < inst.nbE - 1; i++) {
			if(rule == 1)
				model.addConstr(totI[i] >= totI[i+1]);
			if(rule == 2)
				model.addConstr(totS[i] >= totS[i+1]);
			if(rule == 3)
				model.addConstr(totH[i] >= totH[i+1]);
		}
	}
